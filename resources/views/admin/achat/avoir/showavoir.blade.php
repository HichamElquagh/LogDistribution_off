                ...
                <div class="card invoice-preview-card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between flex-xl-row  flex-sm-row flex-column m-sm-3 m-0">
                            ...
                            <div>
                                <h4 class="fw-semibold mb-2">FACTURE AVOIR {{$dataFactureAvoir['numero_avoirsAchat']}}</h4>
                                <div class="mb-4 pt-1 d-flex">
                                    <span class="pe-2">Date: </span>
                                    <span class="fw-semibold pe-3">
                                        {{\Carbon\Carbon::parse($dataFactureAvoir['date_avoirs'])->isoFormat("LL") }}
                                    </span>
                                ...
                                <div class="">
                                    @php
                                        $fournisseurs = Http::get(app('backendUrl').'/fournisseurs/'.$dataFactureAvoir['fournisseur_id']);
                    ...
                    <div class="table-responsive border-top mt-4">
                        <table class="table m-0">
                            ...
                            <tbody>
                                @foreach($dataFactureAvoir['Articles'] as $article)
                                    <tr>
                                ...
                                <tr>
                                    ...
                                    <td class="ps-2 pe-5 py-4 text-end" width="800">
                                        <p class="fw-semibold mb-2 pt-3">{{number_format($dataFactureAvoir['Total_HT'], 2, ',', ' ')}} Dhs</p>
                                        <p class="fw-semibold mb-2">{{number_format($dataFactureAvoir['remise'], 2, ',', ' ')}} Dhs</p>
                                        <p class="fw-semibold mb-2">{{number_format($dataFactureAvoir['Total_TVA'], 2, ',', ' ')}} Dhs</p>
                                        <p class="fw-semibold mb-0 pb-3">{{number_format($dataFactureAvoir['Total_TTC'], 2, ',', ' ')}} Dhs</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="card-body mx-3">
                        <div class="row">
                            <div class="col-12 text-center">
                                ...
                                <span>{{$dataFactureAvoir['Commentaire']}}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-xl-3">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        {{-- @if( $dataFactureAvoir['facture_id'] != null )
                            <a href="{{ route('showFacture', $dataFactureAvoir["facture_id"] )}}" id="goFacture" class="btn btn-light fw-bold text-secondary mb-2 col-12">Facture</a>
                        @endif --}}
                        <a href="{{ route('showRetour', $dataFactureAvoir['bonRetourAchat_id'] )}}" id="retourBonChange" class="btn btn-warning fw-bold text-white col-12">Bon Retour</a>
                ...
                <div class="card" id="imageCard">
                    ...
                    <div class="card-body">
                        ...
                        @if( $dataFactureAvoir['attachement'] == null )
...
$(document).ready(function() {
    ...
    let confirme = {{ $dataFactureAvoir['Confirme'] }};
    let isLinked = {{ $dataFactureAvoir['isLinked']}}; 
    let ttcFacture = {{ $dataFactureAvoir['Total_TTC']}};
    let $statutBadge = $('.statut-dispo');
    let $badgeFacture = $('.statut-paye');
    let factureId = {{ $dataFactureAvoir["id"] }};
    let imageName = '{{ $dataFactureAvoir['attachement'] }}';
    ...
    afficherBadgePaiement(isLinked);

    function afficherBadgePaiement(isLinked) {

        if (isLinked == 0) {
            $badgeFacture.html('<span class="fw-bold">Statut :</span><span class=" badge bg-danger text-white ms-2"><i class="ri-close-circle-line align-middle font-size-14 text-white pe-1"></i> En Attente</span>');
        } else if (isLinked == 1) {
            $badgeFacture.html('<span class="fw-bold">Statut :</span><span class=" badge bg-info text-white ms-2"><i class="ri-radio-button-line align-middle font-size-14 text-white pe-1"></i> Réglé</span>');         
        }
    }
    ...
    $('#confirmationButton').on('click', function() {
        ...
        swal({
            ...
            if (confirm) {
                $.ajax({
                    url: backendUrl + '/avoirsachat/confirme/' + factureId,
                    method: 'PUT',
